...
//	}
	...
	@RequestMapping(value = "/getCreditNoteStatus/{sdNo}", method = RequestMethod.GET)
	@ResponseBody
	public List<CreditNoteStatusDto> getCreditNoteStatus(@PathVariable(value = "sdNo") String sdNo) {
		return creditNoteService.getCreditNoteStatus(sdNo);
	}
	
	@RequestMapping(value = "/callCreditNote/{sdNo}", method = RequestMethod.GET)
	@ResponseBody
	public String callCreditNote(@PathVariable(value = "sdNo") String sdNo) {
		return wfTraackerService.callCreditNoteStatus(sdNo);
	}
}
